...
static if (DMDF_CHARACTER_STATS) then
		...
		*/
		...
		public static method storedCharactersNextIndex takes nothing returns integer
			...
			set cache = InitGameCache("Die Macht des Feuers")
			set index = GetStoredInteger(cache, "Data", "Index")
			set cache = null
			return index
		endmethod

		/// Single player mode allows you to store and restore characters in and from game cache.
		public static method storeCharacter takes Character character, integer index returns boolean
			local gamecache cache
			local integer id
			call ReloadGameCachesFromDisk()
			set cache = InitGameCache("Die Macht des Feuers")
			if (GetStoredInteger(cache, "Data", "Index") > index) then
				call character.displayMessage(ACharacter.messageTypeInfo, IntegerArg(tr("Überschreibe Charakter unter Index %i."), index))
			else
				call character.displayMessage(ACharacter.messageTypeInfo, IntegerArg(tr("Speichere Charakter unter Index %i."), index))

			endif
			call character.store(cache, "Character" + I2S(index))
			call StoreInteger(cache, "Data", "Index", index + 1)

			if (not SaveGameCache(cache)) then
				call character.displayMessage(ACharacter.messageTypeError, tr("Fehler beim Speichern des Charakters auf der Festplatte."))
				return false
			endif
		...
		public static method restoreCharacter takes Character character, integer index, real x, real y, real facing returns boolean
			local gamecache cache
			local integer id
			call ReloadGameCachesFromDisk()
			set cache = InitGameCache("Die Macht des Feuers")
			if (GetStoredInteger(cache, "Data", "Index") <= index) then
				call character.displayMessage(ACharacter.messageTypeError, IntegerArg(tr("Charakter unter Index %i existiert nicht."), index))
				return false
			else
				call character.displayMessage(ACharacter.messageTypeInfo, IntegerArg(tr("Lade Charakter unter Index %i."), index))

			endif
			call character.restore(cache, "Character" + I2S(index), x, y, facing)

			return true
		endmethod

		/// Map name should be generated automatically by using this method and the actual map name. Those characters allow players to see which modes are enabled in this map compilation.
		public static method mapName takes string name returns string
			local string suffix = ""
