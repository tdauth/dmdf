library StructGameGame requires Asl, StructGameCharacter, StructGameItemTypes, StructGameRoutines, StructGameTreeTransparency, LibraryGameLanguage
	...
	struct GameExperience
		...
		private static constant real xpHandicap = 0.50 //0.20 // this XP factor is used to reduce the actual gained experience such as in the Bonus Campaign. We want to prevent the characters from leveling too fast. In the Bonus Campaign it is 10 % and on difficulty hard it is even 7 %.
		...
		public static method unitExperienceForCharacter takes Character character, unit whichUnit, unit killingUnit returns integer
			...
			return R2I(result / ACharacter.countAllPlaying())
		endmethod
