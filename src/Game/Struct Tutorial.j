library StructGameTutorial requires Asl, StructGameCharacter, StructGameSpawnPoint
	...
	struct Tutorial
		...
		private static method onInit takes nothing returns nothing
			...
			call QuestItemSetDescription(questItem, tr("Das Untermenü \"Aufträge\" erlaubt die Auswahl der Zielpunkte von Aufträgen."))
			set questItem = QuestCreateItem(whichQuest)
			call QuestItemSetDescription(questItem, tr("Das Untermenü \"Dungeons\" erlaubt die Kameraansicht eines Dungeons."))
			set questItem = QuestCreateItem(whichQuest)
			call QuestItemSetDescription(questItem, tr("Das Untermenü \"Kamera\" erlaubt das Ändern der Kameraansicht."))
			set questItem = QuestCreateItem(whichQuest)
			call QuestItemSetDescription(questItem, tr("Das Untermenü \"Optionen\" erlaubt die Einstellung von Spieloptionen."))
			...
			call QuestSetDescription(whichQuest, tr("Gegenstände können vom Boden eingesammelt, von Läden gekauft, aus Belohnung für einen Auftrag erhalten oder von einem anderen Charakter erhalten werden. Wird ein Gegenstand von einem Unhold für einen Spieler fallen gelassen, so gehört er diesem Spieler. Dies ist durch die Farbe des Gegenstandes gekennzeichnet. In diesem Fall kann nur der Charakter des Spielers den Gegenstand einsammeln."))
			call QuestSetIconPath(whichQuest, "ReplaceableTextures\\CommandButtons\\BTNChestOfGold.blp")
			set questItem = QuestCreateItem(whichQuest)
			call QuestItemSetDescription(questItem, tr("Gegenstände können im Rucksack des Charakters verstaut werden."))
			set questItem = QuestCreateItem(whichQuest)
			call QuestItemSetDescription(questItem, tr("Gegenstände können mit RM auf den Gegenstand und LM auf den Laden verkauft werden (alle Aufladungen)."))
			set questItem = QuestCreateItem(whichQuest)
			call QuestItemSetDescription(questItem, tr("Manche Gegenstände können mit RM auf den Gegenstand und LM auf den selben Gegenstand ausgerüstet werden."))
			set questItem = QuestCreateItem(whichQuest)
			call QuestItemSetDescription(questItem, tr("Wichtige Gegenstände für Aufträge können weder zerstört noch verkauft werden."))
