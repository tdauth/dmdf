library StructGameTutorial requires Asl, StructGameCharacter, StructGameSpawnPoint
	...
	struct Tutorial
		...
		private static method onInit takes nothing returns nothing
			...
			set questItem = QuestCreateItem(whichQuest)
			call QuestItemSetDescription(questItem, tre("Gegenstände können mit RM auf den Gegenstand und LM auf den Laden verkauft werden (alle Aufladungen).", "Items can be sold with RM and LM on the store (all charages)."))
			set questItem = QuestCreateItem(whichQuest)
