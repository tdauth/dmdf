library StructMapTalksTalkSisgard requires Asl, StructGameCharacter, StructGameClasses, StructMapMapNpcs, StructMapMapFellows, StructMapQuestsQuestTheMagic, StructMapQuestsQuestTheMagicalShield, StructMapQuestsQuestTheGhostOfTheMaster
	...
	struct TalkSisgard extends Talk
		...
		private static method infoActionBeenAtTheRunes_Magic takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Hmm, dann solltest du mal einen Heiler aufsuchen."), gg_snd_Sisgard25)
			call speech(info, character, true, tr("Tut mir leid, aber ich habe dich belogen, um zu sehen, ob du eine ehrliche Haut bist. Mit unehrlichen Leuten werde ich sicherlich nicht gemeinsam umher ziehen."), gg_snd_Sisgard26)
		...
		private static method infoActionBeenAtTheRunes_Nothing takes AInfo info, Character character returns nothing
			...
			call speech(info, character, true, tr("Ich hatte nichts anderes erwartet. Das war nur ein Test, um zu sehen, ob du eine ehrliche Haut bist."), gg_snd_Sisgard27)
			call speech(info, character, true, tr("Von mir aus können wir jederzeit gemeinsam losziehen."), gg_snd_Sisgard28)
		...
		private static method infoActionMakeGood takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Vielleicht, immerhin will ich nicht nachtragend da stehen auch wenn du mich schwer enttäuscht hast."), gg_snd_Sisgard29)
			call speech(info, character, true, tr("Ist es dir dies mal wenigstens ernst?"), gg_snd_Sisgard30)
			call speech(info, character, false, tr("Ja."), null)
			call speech(info, character, true, tr("Gut, ich gebe dir eine zweite und aller letzte Chance mein Vertrauen zu gewinnen. Es gibt da eine wichtige Sache, die ich noch erledigen müsste."), gg_snd_Sisgard31)
			call speech(info, character, true, tr("Wie du sicherlich weißt, steht uns ein Krieg gegen die Orks und Dunkelelfen unmittelbar bevor. Ganz Talras spricht von nichts anderem mehr und ich als „angesehene“ Bürgerin möchte natürlich meinen Beitrag zur Verteidigung leisten."), gg_snd_Sisgard32)
			call speech(info, character, true, tr("Ich habe einen neuen Zauber kreiert, der einen mächtigen magischen Schild erschaffen soll, welcher uns vor Pfeilangriffen schützt. Um ihn auszuprobieren, müsstest du dich allerdings von echten Pfeilen beschießen lassen."), gg_snd_Sisgard33)
			call speech(info, character, false, tr("Ist das dein Ernst?"), null)
			call speech(info, character, true, tr("Tja, vielleicht hättest du mich nicht belügen sollen …"), gg_snd_Sisgard34)
			call speech(info, character, false, tr("Schon gut, also was genau muss ich tun?"), null)
			call speech(info, character, true, tr("Du wirkst zunächst den Zauber, der den magischen Schild erzeugt und dann einen weiteren Zauber, der Pfeile auf dich los lässt und dann wartest du einfach ab was passiert."), gg_snd_Sisgard35)
			call speech(info, character, false, tr("Das hört sich gefährlich an."), null)
			call speech(info, character, true, tr("Das ist es auch, deshalb habe ich den Zauber noch nicht ausprobiert. Sei aber nicht so dumm und wirke die Zauber in falscher Reihenfolge. Hier hast du die beiden Zauberspruchrollen."), gg_snd_Sisgard36)
			call speech(info, character, true, tr("Wenn du diesen Auftrag erfüllst, hast du mein Vertrauen wieder erlangt, aber belüge mich dies mal nicht!"), gg_snd_Sisgard37)
			call speech(info, character, false, tr("Wenn der Zauber nicht funktioniert dann sehen wir uns wohl kaum wieder."), null)
			call speech(info, character, true, tr("Habe etwas mehr Vertrauen in meine Zauberkünste!"), gg_snd_Sisgard38)
		...
		private static method infoActionTriedTheShield takes AInfo info, Character character returns nothing
			...
			call speech(info, character, true, tr("Und du lebst noch, also muss es funktioniert haben!"), gg_snd_Sisgard39)
			call speech(info, character, false, tr("Ja, das hat es. Ein Glück für mich!"), null)
			call speech(info, character, true, tr("Mein Meister wäre sicher stolz auf mich, verdanke ich es doch seiner Weisheit diesen Zauber erschaffen zu haben. Wir werden es diesen Orks und Dunkelelfen zeigen, das sage ich dir!"), gg_snd_Sisgard40)
			call speech(info, character, false, tr("Und ziehst du jetzt mit mir in den Kampf?"), null)
			call speech(info, character, true, tr("Selbstverständlich, du hast mein Vertrauen wieder erlangt. Aber glaube ja nicht, dass du mich hättest belügen können!"), gg_snd_Sisgard41)
			call speech(info, character, true, tr("Ich habe die Zauberspruchrollen natürlich präpariert, um zu erfahren, ob du sie richtig gewirkt hast und wie ich sehe hast du mich nicht belogen. Von mir aus können wir jederzeit gemeinsam losziehen."), gg_snd_Sisgard42)
		...
		private static method infoActionMoreHelp takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Hm, ich weiß nicht ob ich dich damit belasten oder dir das überhaupt anvertrauen kann aber …"), gg_snd_Sisgard43)
			call speech(info, character, false, tr("Nun komm schon."), null)
			call speech(info, character, true, tr("Also … ich weiß nicht so recht."), gg_snd_Sisgard44)
			call speech(info, character, false, tr("…"), null)
			call speech(info, character, true, tr("Ich habe dir doch von meinem Meister erzählt und davon wie viel er mir bedeutet hat und ja eigentlich immer noch bedeutet."), gg_snd_Sisgard45)
			call speech(info, character, false, tr("Ja?"), null)
			call speech(info, character, true, tr("Du musst wissen auf unseren Reisen begegneten wir vielen eigenartigen Gestalten. Darunter war auch ein Nekromant, ein Diener Deranors des Schrecklichen. Wir begegneten dem Nekromanten auf unserer Reise vorbei an den Todessümpfen, dort wo Deranors Reich beginnt."), gg_snd_Sisgard46)
			call speech(info, character, true, tr("Es war ein mächtiger Nekromant, der auch unter Deranor gedient und von ihm gelernt hatte. Da wir ihn vor einer üblen Kreatur beschützten, schenkte er uns aus Dankbarkeit einige wertvolle Artefakte und …"), gg_snd_Sisgard47)
			call speech(info, character, false, tr("Und was?"), null)
			call speech(info, character, true, tr("Er brachte uns eine Zauberformel bei, die Geister herbeirufen konnte. Ich weiß nicht, ob er damals in die Zukunft sehen konnte, denn kurze Zeit später kamen wir nach Talras und mein Meister starb schließlich."), gg_snd_Sisgard48)
			...
			call speech(info, character, true, tr("Ich weiß es nicht. Ehrlich gesagt fürchte ich mich davor. Es ist inzwischen so viel Zeit vergangen und ich fürchte es würde meine guten Erinnerungen an ihn zerstören."), gg_snd_Sisgard49)
			call speech(info, character, true, tr("Stell dir nur mal vor was passieren würde, wenn der Zauber nicht funktioniert oder dem Geist meines Meisters Schaden zufügt? Das könnte ich mir nie verzeihen!"), gg_snd_Sisgard50)
			call speech(info, character, false, tr("Aber so wirst du ihn nie wieder sehen."), null)
			call speech(info, character, true, tr("Ich weiß, ach du hast die Erinnerungen in mir wach gerufen jetzt kann ich an nichts anderes mehr denken!"), gg_snd_Sisgard51)
			call speech(info, character, false, tr("Dann probieren wir es doch aus!"), null)
			call speech(info, character, true, tr("Vielleicht hast du Recht. Du hattest auch damit Recht, dass ich wieder los ziehen sollte Abenteuer zu erleben."), gg_snd_Sisgard52)
			call speech(info, character, true, tr("Also gut wir probieren es aus, wenn du willst. Mein Meister liegt begraben auf dem kleinen Friedhof unten beim Bauernhof, dort wo alle Leute aus Talras begraben werden, bis auf die Familie des Herzogs."), gg_snd_Sisgard53)
			call speech(info, character, true, tr("Geh mit mir zum Friedhof und ich werde die Zauberformel ausprobieren."), gg_snd_Sisgard54)
		...
		private static method infoActionMetYourMaster takes AInfo info, Character character returns nothing
			...
			call speech(info, character, true, tr("Ja, das habe ich! Ich kann dir gar nicht sagen wie sehr ich nun in deiner Schuld stehe Fremder."), gg_snd_Sisgard55)
			call speech(info, character, true, tr("Ich schenke dir zum Dank dieses mächtige Artefakt, das uns der Nekromant damals überreichte. Es soll von nun an dir gehören."), gg_snd_Sisgard56)
			call speech(info, character, true, tr("Du hast mir Erlösung gebracht und dafür danke ich dir auf ewig!"), gg_snd_Sisgard57)
		...
		private static method infoActionLetsGo takes AInfo info, Character character returns nothing
			...
			if (info.talk().characters().size() > 1) then
				call speech(info, character, true, tr("Ich unterhalte mich gerade noch. Danach kann ich dir folgen."), null) // TODO missing sound
				call info.talk().showStartPage(character)
			// (Unterhält sich nur mit dem aktuellen Charakter)
			else
				call speech(info, character, true, tr("Na wenn du mich so nett bittest. Kann's gar nicht erwarten. Gehe voraus, ich folge dir."), gg_snd_Sisgard58)
		...
		private static method infoActionOnMyOwn takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Gut, du weißt ja, wo du mich findest."), gg_snd_Sisgard59)
			call info.talk().close(character)
		...
		private static method infoActionBusy takes AInfo info, ACharacter character returns nothing
			call speech(info, character, true, tr("Ich habe zu tun."), gg_snd_Sisgard60)
			call info.talk().close(character)
		endmethod
