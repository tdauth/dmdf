	...
	struct TalkRicman extends ATalk
		...
		private static method infoActionWhatAreYouDoingHere takes AInfo info, ACharacter character returns nothing
			...
			// (Falls der Charakter Wigberht noch nicht das Gleiche gefragt hat)
				...
				call speech(info, character, true, tr("Auf einen Dunkelelf, dem ich dann seine Eingeweide herausreiße."), null)
		...
		private static method infoActionWeWon takes AInfo info, Character character returns nothing
			...
			call speech(info, character, true, tr("Das haben wir und ihr habt euch tapfer geschlagen. Dennoch war es nur eine kleine Vorhut."), null)
			call speech(info, character, true, tr("Wir müssen uns gut vorbereiten wenn wir das Heer der Orks und Dunkelelfen schlagen wollen."), null)
			call speech(info, character, true, tr("Zum Dank für deine Dienste möchte ich dir diesen mächtigen Stab überreichen."), null)
			call speech(info, character, true, tr("Er stammt aus einer alten Festung weit oben im Norden. Man sagt ein Magier habe ihn geschaffen, der einst die Drachen beherrschte."), null)
			call speech(info, character, true, tr("Ich hoffe er wird dir von großem Nutzen sein. Der Legende nach kann man mit diesem Stab einen gezähmten Drachen beschwören, es handelt sich jedoch nur um ein Gerücht."), null)
			call speech(info, character, true, tr("Probiere ihn doch einmal aus und berichte mir dann, ob es funktioniert."), null)
		...
		private static method infoActionSummonedDragon takes AInfo info, Character character returns nothing
			...
			call speech(info, character, true, tr("Tatsächlich? Unglaublich dass es funktioniert hat."), null)
			call speech(info, character, true, tr("Du musst wissen, in den alten Legenden heißt es weiter, dass der Magier sogar auf den Drachen geritten ist, um dort hin zu gelangen wo auch immer er hin wollte."), null)
			call speech(info, character, true, tr("Probiere das doch einmal aus."), null)
		...
		private static method infoActionDragonEggs takes AInfo info, Character character returns nothing
			...
			call speech(info, character, true, tr("Vielen Dank, das werde ich dir nicht vergessen!"), null)
		...
		private static method infoActionCanYouTeachMe takes AInfo info, ACharacter character returns nothing
			...
			if (character.class() == Classes.dragonSlayer()) then
				call speech(info, character, true, tr("Du bist ein Drachentöter, ein wahrer Krieger. Gib mir ein paar Goldmünzen und ich zeige dir ein paar nette Tricks!"), null)
