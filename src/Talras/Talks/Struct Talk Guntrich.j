library StructMapTalksTalkGuntrich requires Asl, StructMapMapNpcs, StructMapQuestsQuestWitchingHour
	...
	struct TalkGuntrich extends ATalk
		...
		private static method infoAction0 takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Hallo Fremder. Kannst du mir vielleicht weiterhelfen?"), null)
		...
		private static method infoAction1 takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Natürlich, ich bin ja nicht umsonst Müller. Außer Mehl noch ein paar andere nützliche Dinge, um mir etwas dazu zu verdienen."), null)
		...
		private static method infoAction2 takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Ich höre sie trommeln und Schlachtengetümmel. Ich sag's dir, der Berg ist verflucht!"), null)
			call speech(info, character, true, tr("Es klingt als ob zwei Heere aufeinander zustürmten und ewig gegeneinander kämpften. Einmal hab ich mir das angetan, ein zweites Mal kriegt mich keiner auf diesen verfluchten Berg!"), null)
			call info.talk().showStartPage(character)
		endmethod
		...
		private static method infoAction3 takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("War doch klar! Dieser miese Hund hat nur böses im Sinn. Seine Habgier wird ihm nochmal zum Verhängnis werden."), null)
			call speech(info, character, true, tr("Trotzdem danke für die Mühe. Hier hast du ein paar Goldmünzen."), null)
			...
			call speech(info, character, true, tr("Wenn du das machen würdest, wäre ich dir sehr verbunden. Ich werde dich aber sicher nicht als einen Feigling bezeichnen, wenn du es dir nicht zutraust."), null)
		...
		private static method infoAction4 takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("In der Trommelhöhle? Ich habe Geschichten darüber gehört. Angeblich nur ein böses Kindermärchen. Es gibt sie also wirklich?"), null)
			...
			call speech(info, character, true, tr("Hm ... gut, ich vertraue dir. Vielen Dank. Jetzt kann ich wieder beruhigt zu meiner Mühle gehen, auch wenn mich diese Geräusche stören. Hier hast du eine kleine Stärkung und ein paar Goldmünzen."), null)
		...
		private static method infoAction0_0 takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Erstmal hallo. Ich bin Guntrich, der Müller hier. Eigentlich wäre ich ja jetzt auch bei meiner Mühle."), null)
			call speech(info, character, true, tr("Leider ist der Berg anscheinend von bösen Geistern umgeben und die spuken dort Tag und Nacht. Darum bin ich jetzt hier, auf der Suche nach Hilfe."), null)
			call speech(info, character, true, tr("Ich war schon bei diesem verdammten Burgkleriker und der hat mir was von beten erzählt. Mit Beten bekämpft man aber doch keine Geister."), null)
			call speech(info, character, true, tr("Vielleicht kannst du ja mal mit ihm reden. Wenn ich nicht zu meiner Mühle kann, kann ich auch kein Mehl herstellen und das ist mein Ruin!"), null)
		...
		private static method infoAction0_1 takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Verdammt! Du bist auch nicht anders als die Anderen."), null)
			call info.talk().showStartPage(character)
		endmethod
