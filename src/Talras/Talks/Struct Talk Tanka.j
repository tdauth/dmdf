library StructMapTalksTalkTanka requires Asl, StructMapTalksTalkBrogo, StructMapQuestsQuestShamansInTalras
	...
	struct TalkTanka extends Talk
		...
		private static method infoAction8 takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Du hast die Spur der Bestie gefunden."), gg_snd_Tanka21)
			// (Auftrag „Katzen für Brogo“ ist abgeschlossen)
			if (QuestCatsForBrogo.characterQuest(character).isCompleted()) then
				call speech(info, character, true, tr("Außerdem hast du Brogo Katzen geschenkt."), gg_snd_Tanka22)
				call speech(info, character, true, tr("Du bist ein wahrer Freund und ich werde dich alles lehren, was ich weiß. Dennoch verlange ich ein paar Goldmünzen dafür. Ich muss schließlich auch von etwas leben, also nimm's bitte nicht persönlich."), gg_snd_Tanka23)
			// (Auftrag „Katzen für Brogo“ ist nicht abgeschlossen)
			else
				call speech(info, character, true, tr("Das reicht mir als Beweis, dass du in Ordnung bist. Gut, ich werde dir ein paar Sachen beibringen. Aber die wirklich nützlichen Dinge erfährst du erst, wenn ich dir wie einem guten Freund vertraue. Das Ganze kostet dich allerdings auch eine Kleinigkeit."), gg_snd_Tanka24)
			endif
			call info.talk().showStartPage(character)
		endmethod
		...
		private static method infoActionYouKnowTellborn takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Wen?"), gg_snd_Tanka29)
			call speech(info, character, false, tr("Tellborn, einen Schamanen, der sich weiter südlich von hier aufhält."), null)
			call speech(info, character, true, tr("Ich bin ja nicht gerade lange in dieser Gegend, daher kenne ich auch niemanden hier."), gg_snd_Tanka30)
			call speech(info, character, true, tr("Aber sag, ist er wirklich ein Schamane oder behauptet er das nur von sich?"), gg_snd_Tanka31)
		...
		private static method infoActionLetterFromTellborn takes AInfo info, Character character returns nothing
			...
			if (QuestShamansInTalras.characterQuest(character).questItem(1).isCompleted()) then
				call speech(info, character, true, tr("Er hat sich also die Mühe gemacht zu antworten. Immerhin! Nun gib schon her, ich will wissen, was er schreibt."), gg_snd_Tanka37)
				...
				call speech(info, character, true, tr("Vielen Dank! Zu gerne will ich wissen, was hinter ihm steckt. Hier nimm dies als Belohnung."), gg_snd_Tanka38)
			...
			// (Charakter kommt zum ersten Mal im Aufrag von Tellborn)
				...
				call speech(info, character, true, tr("Von wem?"), gg_snd_Tanka39)
				...
				call speech(info, character, true, tr("Wie bitte? Wie konntest du … was schreibt er denn?"), gg_snd_Tanka40)
				call speech(info, character, false, tr("Hier, lies selbst."), null)
				// „Brief an Tanka“ übergeben
				call character.inventory().removeItemType('I03K')
				call speech(info, character, true, tr("(Liest) Aha, unglaublich! Ich muss ihm sofort antworten. Lauf ja nicht weg!"), gg_snd_Tanka41)
				call speech(info, character, false, tr("Schon gut."), null)
				call speech(info, character, true, tr("(Verfasst ein Schreiben)"), null)
				call speech(info, character, true, tr("So, da hast du ein Schreiben für ihn. Bring es ihm so schnell wie möglich!"), gg_snd_Tanka42)
		...
		private static method infoActionYouKnowTellborn_Sure  takes AInfo info, Character character returns nothing
			...
			call speech(info, character, true, tr("Ich danke dir! Warte kurz bis ich das Schreiben aufgesetzt habe."), gg_snd_Tanka34)
			call speech(info, character, true, tr("(Setzt ein Schreiben auf)"), null)
			call speech(info, character, true, tr("So, hier hast du es. Mal sehen, was er dazu sagt …"), gg_snd_Tanka35)
		...
		private static method infoActionYouKnowTellborn_No takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Schade, überlege es dir doch noch einmal."), gg_snd_Tanka36)
			call info.talk().showStartPage(character)
		endmethod
