library StructMapTalksTalkOsman requires Asl, StructGameClasses, StructMapMapNpcs, StructMapQuestsQuestWitchingHour, StructMapQuestsQuestTheDarkCult
	...
	struct TalkOsman extends Talk
		...
		private static method infoActionArea takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tre("Seitdem lebe ich hier in der Burg und studiere die Götter und ihre Geschichte.", "Since then I live here in the castle, studying the gods and their past."), null)
			call speech(info, character, true, tre("Die Leute in dieser Gegend sind dem Glauben sehr verbunden, besonders die Familie des Herzogs. Aber auch auf dem Bauernhof sind die Menschen äußerst fromm.", "The folks in this strongly appreciate their faith, exepcially the Duke's familty. But also the people on the farm here are utterly devout."), null)
			call speech(info, character, true, tre("Im Norden und Nordosten jedoch befinden sich finstere Wälder, geplagt von Unheil und Abtrünnigen des Glaubens.", "Though, in the north and northeast are dark forests marked by mischief and renegarded of faith."), null)
			call speech(info, character, true, tre("An deiner Stelle würde ich diese Gegenden meiden.", "If I was you I would avoid those places."), null)
		...
		private static method infoActionHelp takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tre("Hm, es gibt da etwas, was mir auf dem Herzen liegt seit ich nach Talras gekommen bin, vor so vielen Jahren.", "Hm, there's something on my mind since I came to Talras, so many years ago."), null)
			call speech(info, character, false, tre("Und das wäre?", "What is it?"), null)
			call speech(info, character, true, tre("Du musst wissen, die Bewohner von Talras sind womöglich nicht die einzigen Menschen, die sich hier herumtreiben. Damals als ich in Talras ankam gab es eine Auseinandersetzung unter den Bewohnern.", "You have to know that the residents of Talras are probably not the only people in this area. At the time I arrived in Talras there was a dispute among the living residents there."), null)
			call speech(info, character, true, tre("Ein Teil der Bewohner spaltete sich vom damaligen Herzog, dem Vater Heimrichs, ab und verließ die Burg und den Bauernhof.", "A part of the people splitted from the Duke back then, Heimrich's father, and left the castle and the farm beyond."), null)
			call speech(info, character, true, tre("Sie schworen den Göttern ab und zogen in den Wald, um etwas ... anderem zu dienen.", "They forswear the gods und settled down in the forest to serve... something else."), null)
			call speech(info, character, true, tre("Der dunkle Kult belästigte daraufhin die Bewohner der Burg und des Dorfes. Sie stahlen, verschleppten und mordeten sogar bis schließlich der Herzog selbst ausritt und dem Ganzen ein Ende bereitete.", "Their new dark cult was disturbing the peace of of the castle's inhabitants and the ones in the whole village. They stole, kidnapped, and murdered until the Duke himself  finaly dealt with the issue to end all the drama."), null)
			call speech(info, character, true, tre("Wie du dir denken kannst, wurden nicht alle Anhänger des Kultes gefasst und erschlagen. Ich frage mich seitdem, ob der Kult danach seine Existenz aufgegeben hat, haben wir doch so lange nichts mehr von ihm gehört.", "But as you probably can imagine not everyone of the new cult was caught and slayed. Since then I ask myself if the cult resigned it's existence, since we haven't heard of them for a long time already."), null)
			call speech(info, character, true, tre("Diese ungeklärte Frage muss ich wohl mit ins Grab nehmen.", "Seems like I will take this unexplained question right with me into the grave."), null)
		...
		private static method infoActionMoreAboutTheCult takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tre("Sie vergingen sich an Dorfbewohnern in einer Art und Weise, die selbst erfahrene Kriegsleute abschrecken würde.", "They dealt with other vilagers in such a mannar that it even experienced soldiers would be scared off."), null)
			call speech(info, character, true, tre("Die Leichen schleppten sie danach mit in ihr Versteck. Die Götter allein wissen, was sie damit vor hatten.", "The dead bodies were braught into their hide. The gods alone know what they were about to do with them."), null)
		...
		private static method infoActionFoundTheCult takes AInfo info, Character character returns nothing
			...
			call speech(info, character, true, tre("Wie bitte?! Bei den Göttern, dagegen müssen wir sofort vorgehen!", "How is this possible!? By all gods, we immediatly have to act against this!"), null)
			...
			call speech(info, character, true, tre("Wie meinst du das?", "What do you mean?"), null)
