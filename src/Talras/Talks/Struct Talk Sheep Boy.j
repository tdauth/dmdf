library StructMapTalksTalkSheepBoy requires Asl, StructMapMapNpcRoutines, StructMapMapNpcs, StructMapQuestsQuestWolvesHunt, StructMapQuestsQuestStormingTheMill
	...
	struct TalkSheepBoy extends ATalk
		...
		private static method infoActionHi takes AInfo info, ACharacter character returns nothing
			call speech(info, character, true, tr("Was machst du hier? Willst du etwa ein Schaf stehlen? Sieh dich bloß vor, sonst kriegst du es mit mir zu tun!"), null)
		...
		private static method infoActionWhoAreYou takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Ich bin der Schafsjunge auf dem Bauernhof."), null)
			call speech(info, character, true, tr("Ich bewache Manfreds Schafe, damit sie ungestört grasen können und bis zum Winter ordentlich fett werden und gesund bleiben."), null)
			call info.talk().showStartPage(character)
		endmethod
		...
		private static method infoActionFear takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Du meinst wie der alte Guntrich dort hinten? Nein, ich glaube nicht an irgendwelche Gespenstergeschichten. Ich fürchte mich vor gar nichts!"), null)
			call speech(info, character, false, tr("Wirklich?"), null)
			call speech(info, character, true, tr("Ja sicher, ob Wegelagerer, Orks oder Dunkelelfen. Die Männer auf dem Bauernhof sind doch allesamt Feiglinge. Ich würde es mit einer ganzen Armee aufnehmen wenn es sein muss."), null)
			call info.talk().showStartPage(character)
		endmethod
		...
		private static method infoActionHelp takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Quatsch, ich brauche sicher keine Hilfe und schon gar nicht von dir!"), null)
			call speech(info, character, false, tr("Sicher?"), null)
			call speech(info, character, true, tr("Ja, sicher! Außer … sag mal bist du ein guter Jäger?"), null)
			call speech(info, character, false, tr("Sicher."), null)
			call speech(info, character, true, tr("In dieser Gegend wimmelt es von Wölfen. Sie haben wahrscheinlich mitbekommen, dass hier das fette Vieh grast und wollen sich die Bäuche vollschlagen."), null)
			call speech(info, character, true, tr("Wenn sie sich den Schafen nähern sollten, dann schlage ich sie in die Flucht."), null)
			call speech(info, character, true, tr("Leider kann ich die Schafe nicht alleine lassen, sonst würde ich diesen Wölfen mal gehörig das Fell über die Ohren ziehen!"), null)
			call speech(info, character, false, tr("Und das soll ich jetzt für dich tun?"), null)
			call speech(info, character, true, tr("Genau, außer du bist so feige wie die anderen hier. Töte am besten ihre Rudelführer, dann sind sie erst mal mit sich selbst beschäftigt."), null)
			call speech(info, character, true, tr("Und denke daran, dass sich nicht nur Wölfe in der Nähe aufhalten. Sie verbreiten sich überall in Talras, schlimmer als die Pest!"), null)
		...
		private static method infoActionWolvesDead takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Du bist also kein Feigling? Das hätte ich auch alleine geschafft … wenn ich nicht dauernd hier aufpassen müsste."), null)
			call speech(info, character, false, tr("Bestimmt."), null)
			call speech(info, character, true, tr("Hier hast du etwas von den Schafen, zum Dank."), null)
		...
		private static method infoActionMoreHelp takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Nicht wirklich. Seit Guntrich nicht mehr zu seiner Mühle geht, haben sich dort hinten Wegelagerer breit gemacht."), null)
			call speech(info, character, true, tr("Die sitzen dort und fressen sich satt an unserem Korn. Wenn Guntrich nichts unternimmt, sind bald unsere gesamten Vorräte weg."), null)
			call speech(info, character, true, tr("Aber er glaubt ja es würde spuken, also muss ich mich wieder darum kümmern."), null)
			call speech(info, character, false, tr("Und was willst du tun?"), null)
			call speech(info, character, true, tr("Sie alle töten, diese verdammten Räuber … aber ich darf die Schafe nicht im Stich lassen."), null)
			call speech(info, character, true, tr("Du bist doch …"), null)
			call speech(info, character, false, tr("Verstehe schon, ich soll sie für dich erledigen."), null)
			call speech(info, character, true, tr("Ja, aber nicht einfach so. Sie sollen wissen, wem dieser Berg gehört und wer ihnen ein Ende bereitet."), null)
			call speech(info, character, false, tr("Und das heißt?"), null)
			call speech(info, character, true, tr("Die Schafe grasen hier in Frieden. Wir können keine Eindringlinge gebrauchen. Die Schafe werden sich rächen!"), null)
			call speech(info, character, true, tr("Du musst sie gemeinsam mit den Schafen angreifen!"), null)
			call speech(info, character, false, tr("Was?!"), null)
			call speech(info, character, true, tr("Ja, nur nicht mit allen Schafen, falls die Verluste zu hoch werden. Ansonsten bleiben uns keine Tiere mehr."), null)
			call speech(info, character, true, tr("Sagen wir ich verkaufe dir ein Schaf. Darauf kannst du dann in die Schlacht reiten."), null)
			call speech(info, character, false, tr("WAS?!"), null)
			call speech(info, character, true, tr("Keine Angst, es ist nicht so teuer wie du denkst. Ich mache dir einen Freundschaftspreis."), null)
			call speech(info, character, true, tr("Nun denn, ziehe dahin für Ruhm und Ehre auf dass die Schafe diesen Berg für sich behaupten können!"), null)
		...
		private static method infoActionBanditsDead takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Tatsächlich? Ich hoffe dein Schaf hat dir als Schlachtross treue Dienste geleistet."), null)
			call speech(info, character, false, tr("Natürlich …"), null)
			call speech(info, character, true, tr("Die Schafe danken dir für deine Ritterlichkeit. Endlich gehört der Berg wieder uns!"), null)
