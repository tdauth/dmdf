library StructMapTalksTalkCarsten requires Asl, StructMapQuestsQuestTheAuthor
	...
	struct TalkCarsten extends Talk
		...
		private static method infoActionHi takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Hallo, ich bin Carsten. Ich bin Autor. Kannst du mir helfen von dieser Insel weg zu kommen?"), gg_snd_Carsten1)
			call speech(info, character, false, tr("Wie bist du überhaupt hier her gekommen?"), null)
			call speech(info, character, true, tr("Mein Verleger hat mich auf diese Insel verbannt, solange bis ich mein Buch vollendet habe. Ich fürchte aber, dass er in der Zwischenzeit verstorben ist."), gg_snd_Carsten2)
			call speech(info, character, false, tr("Und hast du dein Buch vollendet?"), null)
			call speech(info, character, true, tr("Sicher. Es heißt \"Die Dunkelmagierchroniken\" und wird bestimmt das meistverkaufte Werk in den Klöstern des Königreichs. Die einfachen Leute können sowieso nicht lesen, was bedeutet, dass ich es entsprechend den Vorgaben der meisten Klöster schreiben musste."), gg_snd_Carsten3)
			call speech(info, character, false, tr("Und was bedeutet das genau?"), null)
			call speech(info, character, true, tr("Wenig Tiefgründiges und viel Versautes. Das ist doch, was die ganzen Kleriker und Priester wollen. Also hilfst du mir?"), gg_snd_Carsten4)
		...
		private static method infoActionHelp takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Ganz einfach. Besorge mir eine Sprucholle des Weges. Damit sollte ich mich von hier weg teleportieren können."), gg_snd_Carsten5)
		...
		private static method infoActionScroll takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Ich danke dir. Wenn ich mich bereit fühle, werde ich es wagen die Spruchrolle anzuwenden. Diese Insel hat mich beinahe zermürbt. Dafür ist mein Werk umso besser geworden. Hier hast du ein paar Goldmünzen zur Belohnung."), gg_snd_Carsten6) // TODO und gg_snd_Carsten7
		...
		private static method infoActionRead takes AInfo info, ACharacter character returns nothing
			...
			call speech(info, character, true, tr("Was? Bist du des Wahnsinns? Aus meinem vollkommenen, perfekten Werk soll ich DIR vorlesen? Erst wenn es von den Dienern des Klosters vervielfacht wurde, kannst du es vielleicht zu einem Preis von 10 000 Goldmünzen erwerben und dieser Preis läge noch weit unter dem eigentlichen Marktwert."), gg_snd_Carsten8) // TODO und gg_snd_Carsten9
			call speech(info, character, true, tr("In dieses Buch ist meine Leidenschaft geflossen. Zwanzig Jahre musste ich auf dieser Insel verweilen, bis du schließlich kamst. Es ist ein Werk der Zukunft, der Veränderung, es wird eine neue Religion begründen, den Frieden bringen. Es ist ..."), gg_snd_Carsten10) // TODO und gg_snd_Carsten11
