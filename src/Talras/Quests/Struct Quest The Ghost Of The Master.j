library StructMapQuestsQuestTheGhostOfTheMaster requires Asl, StructGameCharacter, StructMapMapNpcs, StructMapMapFellows
	...
	struct QuestTheGhostOfTheMaster extends AQuest
		...
		private static method stateActionCompleted0 takes AQuestItem questItem returns nothing
			...
			call TriggerSleepAction(2.0)
			call waitForVideo(MapData.videoWaitInterval)
			call TransmissionFromUnitForPlayer(questItem.character().player(), master, tre("Wer hat mich aus den Tiefen der Finsternis geweckt? Seid Ihr es Sisgard meine Schülerin?", "Who woke me up from the depths of darkness? Is it you Sigard my student?"), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call waitForVideo(MapData.videoWaitInterval)
			call TransmissionFromUnitForPlayer(questItem.character().player(), Npcs.sisgard(), tre("Meister! Ihr seid es tatsächlich! Ich fasse es nicht Euch nach so langer Zeit wiederzusehen!", "Master! It is you indeed! I cannot believe to see you again after such a long time!"), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call waitForVideo(MapData.videoWaitInterval)
			...
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call waitForVideo(MapData.videoWaitInterval)
			call TransmissionFromUnit(master, tre("Nun gut, jetzt bist du ja hier und wir sehen uns endlich wieder. Ich denke wir haben uns viel zu erzählen, doch besser ohne fremde Augen und Ohren in der Nähe.", "Well, now you're here and we'll see each other at last. I think we have a lot to tell to each other, but better without somebody else's eyes and ears nearby."), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call waitForVideo(MapData.videoWaitInterval)
			call TransmissionFromUnitForPlayer(questItem.character().player(), Npcs.sisgard(), tre("Ihr habt Recht Meister, wie immer! Ich kann Euch nicht sagen wie sehr Ihr mir gefehlt habt.", "You are right master, as always! I cannot tell you how much I've been missing you."), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call waitForVideo(MapData.videoWaitInterval)
			call TransmissionFromUnitForPlayer(questItem.character().player(), master, tre("Nicht jetzt mein Kind, ein andermal! Nun weißt du ja wo und wie du mich finden kannst.", "Not now my child, another time! Now you know where and how to find me."), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call waitForVideo(MapData.videoWaitInterval)
			call TransmissionFromUnitForPlayer(questItem.character().player(), master, tre("Lebe wohl!", "Farewell!"), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call waitForVideo(MapData.videoWaitInterval)
			...
			call TriggerSleepAction(2.0)
			call waitForVideo(MapData.videoWaitInterval)
			call TransmissionFromUnitForPlayer(questItem.character().player(), Npcs.sisgard(), tre("Meister? Das ist doch wieder typisch für ihn! Er ist eben einfach zu menschenscheu.", "Master? That is again typical for him! He's just simply unsociable."), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call waitForVideo(MapData.videoWaitInterval)
