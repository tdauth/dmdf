library StructMapQuestsQuestTheGhostOfTheMaster requires Asl, StructGameCharacter, StructMapMapNpcs, StructMapMapFellows
	...
	struct QuestTheGhostOfTheMaster extends AQuest
		...
		private static method stateActionCompleted0 takes AQuestItem questItem returns nothing
			...
			call TransmissionFromUnitForPlayer(questItem.character().player(), Npcs.sisgard(), tr("Da sind wir, nun lasse ich den Zauber wirken!"), null)
			...
			call TriggerSleepAction(2.0)
			call TransmissionFromUnitForPlayer(questItem.character().player(), master, tr("Wer hat mich aus den Tiefen der Finsternis geweckt? Seid Ihr es Sisgard meine Schülerin?"), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call TransmissionFromUnitForPlayer(questItem.character().player(), Npcs.sisgard(), tr("Meister! Ihr seid es tatsächlich! Ich fasse es nicht Euch nach so langer Zeit wiederzusehen!"), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call TransmissionFromUnitForPlayer(questItem.character().player(), master, tr("Ich bin es Sisgard und ich muss sagen du hättest dich ruhig ein bisschen beeilen können. In der Finsternis ist es nicht gerade spannend und ich hätte eigentlich gedacht ..."), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			...
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call TransmissionFromUnitForPlayer(questItem.character().player(), Npcs.sisgard(), tr("Ihr habt Recht Meister, wie immer! Ich kann Euch nicht sagen wie sehr Ihr mir gefehlt habt."), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call TransmissionFromUnitForPlayer(questItem.character().player(), master, tr("Nicht jetzt mein Kind, ein andermal! Nun weißt du ja wo und wie du mich finden kannst."), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			call TransmissionFromUnitForPlayer(questItem.character().player(), master, tr("Lebe wohl!"), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
			...
			set master = null
			call TriggerSleepAction(2.0)
			call TransmissionFromUnitForPlayer(questItem.character().player(), Npcs.sisgard(), tr("Meister? Das ist doch wieder typisch für ihn! Er ist eben einfach zu menschenscheu."), null)
			call TriggerSleepAction(GetSimpleTransmissionDuration(null))
