...
library StructSpellsSpellBearForm requires Asl, StructGameClasses, StructSpellsSpellMetamorphosis
	...
	struct SpellBearForm extends SpellMetamorphosis
		...
		public stub method onMorph takes nothing returns nothing
			...
			call SetUnitAbilityLevel(this.character().unit(), thistype.lifeAbilityId, Character(this.character()).realSpellLevels().integerByInteger(0, thistype.abilityId))
			call SetUnitAbilityLevel(this.character().unit(), thistype.damageAbilityId, Character(this.character()).realSpellLevels().integerByInteger(0, thistype.abilityId))
		endmethod
